...
func Run(ctx context.Context, conf *Config, runners map[string]bool, c reviewdog.CommentService, d reviewdog.DiffService, teeMode bool, filterMode filter.Mode, failOnError bool, preExistingIssues *map[string]bool) error {
	...
	results.Range(func(toolname string, result *reviewdog.Result) {
		...
		g.Go(func() error {
			...
			return reviewdog.RunFromResult(ctx, c, ds, filediffs, d.Strip(), toolname, filterMode, failOnError, preExistingIssues)
		})
	})
	return g.Wait()
}
